...
function CustomCardPreview({ uploadedImage, cardFinish, displayImage, displayTitle }: { 
  uploadedImage: string | null, 
  cardFinish?: 'matte' | 'rainbow' | 'gloss',
        ...
        >
          ...
          <div className={
            ...
            <div className="relative w-full h-full rounded-xl border-2 border-cyber-cyan/50 shadow-2xl overflow-hidden">
              ...
              <img
                src={displayImage || uploadedImage || ''}
                alt={displayTitle || 'Custom Card'}
...
export function CustomCardCheckoutModal({ isOpen, onClose, uploadedImage, processedImageBlob, uploadedImageUrl, marketplaceListing }: CustomCardCheckoutModalProps) {
  ...
  // Detect if this is a marketplace listing
  const isMarketplaceListing = !!marketplaceListing
  const displayImage = isMarketplaceListing ? marketplaceListing?.image_url : uploadedImage
  const displayTitle = isMarketplaceListing ? marketplaceListing?.title : 'Custom Card'
  const basePrice = isMarketplaceListing ? (marketplaceListing?.price_cents || 0) / 100 : 9.00 // $9.00 for custom cards
  ...
  const getCurrentPricing = () => {
    if (!inventoryState.data) {
      const fallbackPrice = isMarketplaceListing ? basePrice : 9.00
      return {
        pricePerUnit: fallbackPrice, // Use marketplace price or fallback for custom cards
        totalPrice: fallbackPrice * quantityState.value,
        discount: 0,
        savings: 0,
      }
    }

    // For marketplace listings, use simple pricing without tiers
  ...
  const handleAddToCart = async () => {
    ...
    addItem({
      ...
      basePricePerUnit: isMarketplaceListing ? basePrice : (inventoryState.data?.customCard?.pricePerUnit || 9.00),
  ...
  const handleCreateCheckout = async (address: ShippingAddress) => {
    ...
    // For custom cards, we need an uploaded image
    if (!isMarketplaceListing && !uploadedImage) return
    
    // For marketplace listings, we need a listing
    if (isMarketplaceListing && !marketplaceListing) return
    ...
    const logContext = {
      ...
      isCustomCard: !isMarketplaceListing,
    ...
    try {
      ...
      if (isMarketplaceListing) {
        // For marketplace listings, use the listing's image
        customImageUrl = marketplaceListing?.image_url || null
        console.log('🛒 Processing marketplace listing checkout...', {
          listingId: marketplaceListing?.id,
          title: marketplaceListing?.title,
          priceCents: marketplaceListing?.price_cents
        })
      } else {
        // For custom cards, handle image upload
        console.log('📤 Processing custom card checkout...')
                ...
                {currentStep === 'quantity' ? (
                  ...
                  <div className="flex justify-center order-1 lg:order-1">
                    <div className="relative w-full max-w-[200px] sm:max-w-xs lg:max-w-sm">
                      ...
                      >
                        <CustomCardPreview 
                          key={`2d-preview-${displayImage || uploadedImage || 'placeholder'}`} 
